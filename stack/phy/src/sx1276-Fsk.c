...
uint32_t SX1276FskProcess( void )
{
    ...
    case RF_STATE_RX_INIT:
        ...
        Preamble2SyncTimer = RxTimeoutTimer = xTaskGetTickCount( );
    ...
    case RF_STATE_RX_SYNC:
        if( ( DIO4 == 1 ) && ( PreambleDetected == false ) )// Preamble
        {
            ...
            Preamble2SyncTimer = xTaskGetTickCount( ) ;
        }
        ...
        if( ( SyncWordDetected == false ) && ( PreambleDetected == true ) && ( ( xTaskGetTickCount( )  - Preamble2SyncTimer ) > Preamble2SyncTimeout ) )
        {
        ...
        if( ( SyncWordDetected == false ) &&
            ( PreambleDetected == false ) &&
            ( PacketDetected == false ) &&
            ( ( xTaskGetTickCount( )  - RxTimeoutTimer ) > PacketTimeout ) )
        {
    ...
    case RF_STATE_RX_RUNNING:
        ...
        // Packet timeout
        if( ( PacketDetected == false ) && ( ( xTaskGetTickCount( )  - RxTimeoutTimer ) > PacketTimeout ) )
        {
            RFState = RF_STATE_RX_TIMEOUT;
        }
        break;
    ...
    case RF_STATE_TX_READY_WAIT:
        if( DIO3 == 1 )    // TxReady
        {
            ...
            TxTimeoutTimer = xTaskGetTickCount( ) ;
    ...
    case RF_STATE_TX_RUNNING:
        ...
        // Packet timeout
        if( ( xTaskGetTickCount( )  - TxTimeoutTimer ) > TICK_RATE_MS( 1000 ) )
        {

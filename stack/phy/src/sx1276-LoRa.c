 ...
 */
uint8_t RFLRState = RFLR_STATE_TX_INIT;
...
//#if( ( MODULE_SX1276RF1IAS == 1 ) || ( MODULE_SX1276RF1KAS == 1 ) )
    if( LoRaSettings.RFFrequency >= 433050000 )
    {
...
uint32_t SX1276LoRaProcess( void )
{
    ...
    case RFLR_STATE_RX_INIT:
        ...
        else // Rx continuous mode
        {
            ...
            SX1276Write( REG_LR_FIFORXBASEADDR, 0);//SX1276LR->RegFifoAddrPtr );
        ...
        RxTimeoutTimer = xTaskGetTickCount( );
    ...
    case RFLR_STATE_RX_RUNNING:
        ...
        if( LoRaSettings.RxSingleOn == true ) // Rx single mode
        {
            if( ( xTaskGetTickCount( ) - RxTimeoutTimer ) > PacketTimeout )
            {
    ...
    case RFLR_STATE_RX_DONE:
        ...
        if( LoRaSettings.RxSingleOn == true ) // Rx single mode
        {
            ...
            if( LoRaSettings.ImplicitHeaderOn == true )
            {
                RxPacketSize = SX1276LR->RegPayloadLength;
						  	SX1276Read( REG_LR_FIFORXCURRENTADDR, &test1 );
        ...
        else // Rx continuous mode
        {
            ...
            if( LoRaSettings.ImplicitHeaderOn == true )
            {
                ...
                SX1276Write( REG_LR_FIFOADDRPTR, 0);//SX1276LR->RegFifoAddrPtr );
                SX1276ReadFifo( RFBuffer, SX1276LR->RegPayloadLength );
            }
            else
            {
    ...
    case RFLR_STATE_TX_INIT:
        ...
        SX1276LR->RegFifoTxBaseAddr = 0x64; // Full buffer used for Tx
